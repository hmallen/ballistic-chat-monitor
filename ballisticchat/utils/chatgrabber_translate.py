...
if __name__ == "__main__":
    ...
    translate_users = ["Chezi Rubinstein", "GEBELEIZIS", "roni moshe", "Erez Atia"]
    ...
    with open("chat_111120_geb.jsonl", "a+") as dump_file:
        for cnt, doc in enumerate(agg_result):
            ...
            doc["translation"] = ""
            doc["translation_info"] = {}

            if doc["name"] in translate_users:
                try:
                    detected_language = langdetect.detect(doc["text"])
                    logger.debug(f"detected_language: {detected_language}")
                except:
                    logger.debug("Exception on language detection. Skipping.")
                    detected_language = ""

                if detected_language != "en":
                    try:
                        translate_result = translate_client.translate(doc["text"])
                        doc["translation"] = translate_result.text

                        logger.debug(
                            f"Translation: {doc['name']} => {doc['translation']}"
                        )

                    except Exception as e:
                        logger.exception(f"Exception while translating: {e}")
